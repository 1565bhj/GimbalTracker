...
u8 SDRAM_Init(void)
{
    ...
    /* 1. 时钟使能命令 */
	  err=1;
    if(SDRAM_SendCommand(FMC_SDRAM_CMD_CLK_ENABLE, 1, 1, 0))
			return err;
    ...
    /* 3. SDRAM全部预充电命令 */
		err=2;
		if(SDRAM_SendCommand(FMC_SDRAM_CMD_PALL, 1, 1, 0))
			return err;

    /* 4. 自动刷新命令 */
		err=3;
		if(SDRAM_SendCommand(FMC_SDRAM_CMD_AUTOREFRESH_MODE, 1, 8, 0))
			return err;
    ...
    temp = (uint32_t)SDRAM_MODEREG_BURST_LENGTH_1            |          //设置突发长度：1
		...
		if(SDRAM_SendCommand(FMC_SDRAM_CMD_LOAD_MODE, 1, 1, temp))
			return err;
    ...
    */
		err=5;
		if(HAL_SDRAM_ProgramRefreshRate(&hsdram1, 918) != HAL_OK)
			return err;
		return 0;
